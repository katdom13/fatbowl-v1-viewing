...
const Addresses = () => {
  ...
  const handleDelete = (public_id) => {
    deleteAddress(public_id, cookies.csrftoken)
      .then(res => console.log('Address deleted'))
      .catch(err => console.error('[DELETE ADDRESS ERROR]', err && err.response ? err.response : err))

    // Delete from frontend list as well
    const newAddresses = addresses.filter(address => address.public_id !== public_id)
