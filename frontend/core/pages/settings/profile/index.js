...
import { getUser, whoami, updateUser, logoutUser, deleteUser } from "../../../config/axios"
...
const Profile = () => {
  ...
  const {context: {logout}} = useContext(AppContext)
          ...
          updateUser(
            ...
            .then(response => {
              if (Boolean(data.old_password) && Boolean(data.new_password)) {
                ...
                setAlert({severity: 'success', message: 'Successfully updated profile'})
              }
            })
            .catch(err => {
