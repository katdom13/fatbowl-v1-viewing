...
import { createAddress, getAddress, getAddresses, updateAddress } from "../../../config/axios"
  ...
  useEffect(() => {
    ...
    if (Boolean(public_id)) {
      ...
      getAddress(public_id)
        ...
        .catch(err => console.error('[GET ADDRESS ERROR]', err && err.response ? err.response : err))
    }
    ...
    !isEdit ? (
      createAddress(
        ...
        .catch(err => {
          ...
          console.error('[ADD ADDRESS ERROR]', err && err.response ? err.response.data : err)
        })
    ) : (
      updateAddress(
        publicId,
        ...
        .catch(err => {
          ...
          console.error('[EDIT ADDRESS ERROR]', err && err.response ? err.response : err)
        })
    )
